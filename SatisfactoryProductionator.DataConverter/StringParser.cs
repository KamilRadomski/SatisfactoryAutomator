namespace SatisfactoryProductionator.DataConverter
{
	internal static class StringParser
	{
		...
		public static Dictionary<string, double[]> ParseItemsAndQuantityPerMinute(string rawItems, Dictionary<string, string> itemMap, double buildTime)
		{
			...
			foreach (var (name, quantity) in splitItems)
			{
				...
				items.Add(itemMap[name], new[] { buildPerRecipe, buildPerMinute });
		...
		public static Dictionary<string, double> ParseItemsAndQuantity(string rawItems, Dictionary<string, string> itemMap)
		{
			...
			foreach (var (name, quantity) in splitItems)
			{
				buildingCost.Add(itemMap[name], quantity);
			}
